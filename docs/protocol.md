...
**Untrusted `actionId` (by design):** Decode only enforces UTF-8 length. Nothing in the protocol layer or the reference Fabric packet receivers checks that `actionId` appears in a prior `s2c_bindings` payload for that player. **The host server/mod must allow-list** (and rate-limit) key actions against bindings it actually sent—see [Paper Plugin → Security](paper-plugin.md). That separation is intentional: KrepAPI stays transport + dispatch hooks; binding policy belongs in your game code.
